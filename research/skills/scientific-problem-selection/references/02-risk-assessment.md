...
#### Type A: Assumptions About Domain Reality
These are facts about the world that either are or aren't true. They won't change during the project.

**Examples:**
- Users will adopt the proposed design patterns
- The optimization approach will converge to meaningful solutions
- Data of sufficient quality exists or can be collected
- The design principles will transfer across contexts
- The effect size is large enough to be meaningful
...
**Examples:**
- The simulation can run at acceptable speed
- ML models will achieve sufficient accuracy
- VR hardware will support the required interaction
- Computational analysis can distinguish signal from noise
- The algorithm will scale to realistic problem sizes

**Claude should ask:**
1. What must be true about the domain for this to work?
2. What must the technology be able to do?
3. What about the research design—what assumptions are built in?
...
2. Distinguish between domain vs. technical assumptions
...
**Example:** Instead of waiting 2 years to validate the full system works:
- Test core algorithm on simplified toy problems first
- Run pilot study with small user group
- Use proxy metrics that indicate likely success
...
**Example:** Instead of:
- Testing one algorithm → Test a panel of 5 approaches
- Building one design tool → Build and test multiple prototypes
- Pursuing one application domain → Test in 3 related contexts
...
**Example:**
- **Original:** Develop fully automated design generation (high risk: may not meet quality standards)
- **Reframed:** Develop design assistance that augments human designers (lower risk)

#### Strategy 4: Change the Context
**Question:** Is there a different research context with similar scientific value but lower technical risk?

**Example:**
- **Original:** Test with practicing architects (hard to recruit, limited availability)
- **Alternative:** Test with architecture students (easier access, similar design thinking)

#### Strategy 5: Add Complementary Approaches
**Question:** Can a parallel approach be added that de-risks the main assumption?

**Example:**
- Add user studies alongside algorithmic evaluation
- This provides human validation to complement computational metrics
...
2. **The Study:** Most direct test possible (even if crude)
...
Claude should help search academic databases to calibrate risk assessments:

**Search for:**
1. **Precedents:** Has anyone done something similar? (Reduces technical risk)
2. **Domain Evidence:** What's known about the context? (Informs domain risk)
...
- Reorder studies to test high-risk assumptions early
...
|------------|-------|-------|-------|-------|
| [Assumption 1] | Domain/Tech | 1-5 | X mo | [Rationale for score] |
| [Assumption 2] | Domain/Tech | 1-5 | X mo | [Rationale for score] |
| ... | ... | ... | ... | ... |

*Domain = Domain reality, Tech = Technical capability
...
```
Month 0-6:   [Early go/no-go studies]
...
### Example 1: AI-Based Floor Plan Generation

**High-Risk Assumptions Identified:**
1. "Generated layouts will meet professional quality standards" (Risk 4, 18 months)
2. "Users will trust AI-generated options" (Risk 4, 24 months)

**Problem:** Two risk-4 assumptions at 18+ months = YELLOW FLAG

**Mitigation Applied:**
- Reframe to AI-assisted (human-in-the-loop) rather than fully autonomous (reduces Risk 4→2)
- Test with design students first before professionals (earlier validation at 6 months)
- Add comparative study with existing tools (provides benchmark)

### Example 2: VR for Participatory Urban Planning

**High-Risk Assumption Identified:**
"Participants will engage meaningfully with VR environment" (Risk 3, unknown timeline)

**Problem:** Critical assumption with unclear path to resolution

**Mitigation Applied:**
- Start with simpler 3D visualization before full VR immersion
- Add parallel track: test multiple engagement modalities
- Design study where understanding engagement factors IS the outcome (multiple success paths)
...
3. **Early and Often:** Design go/no-go studies at the earliest feasible moment.
...
*Remember: Spending time on risk analysis is the most valuable investment a researcher can make. A well-understood risk profile enables moving forward with confidence or pivoting with clarity—both are valuable outcomes.*
