# Kontakte, Ansprechpartner und Ablauf

## Wichtige Kontakte

### Reisekostenstelle (Dezernat Personal)
- **Email**: Reisekostenabrechnung@uni-weimar.de
- **Telefon**: +49 (0) 36 43 / 58 22 14 oder +49 (0) 36 43 / 58 22 22
- **Ansprechpartnerin**: Beate Haltmeyer-Forstner (beate.haltmeyer-forstner@uni-weimar.de)
- **Zuständig für**: Alle Fragen zu Reisekosten, Abrechnung, BahnCard, Abschläge

### Dekanat Fakultät A+U
- **Claudia Rehm** — Teamassistenz Dekanat
  - Tel.: +49 (0) 3643 583112
  - **Zuständig für**: Entgegennahme der DR-Anträge, Weiterleitung an Dekan, Rücksendung der genehmigten Anträge (seit 18.03.2026 digital per Email)
- **Dr. Anna Scheer** — Geschäftsführung
  - Tel.: +49 (0) 36 43 / 58 31 13
  - Email: anna.scheer@uni-weimar.de
### InfAU Office Management
- **Franziska Schuchort** — Office Managerin InfAU
  - Email: caad@architektur.uni-weimar.de
  - **Zuständig für**: Entgegennahme und Weiterleitung von DR-Anträgen für InfAU-Mitarbeitende. Anträge an Franzi geben oder per Email senden — sie leitet an das Dekanat weiter. Bei Email-Einreichung Franzi mindestens in CC setzen.

### Fahrkartenbestellung
- BUW-Bucher*innen (für Bahnfahrkarten, außer VMT)
- Info: https://www.uni-weimar.de/de/fs/uni-intern/henri/alphabetisches-verzeichnis/henri-klemm/dienstreisen-fahrkartenbestellung/

---

## Digitalisierung ab 18.03.2026

Seit dem **18.03.2026** gilt die vollständig digitale Einreichung:

### Was sich geändert hat
1. **Keine Papiereinreichung mehr** — alles per Email
2. **Dekanat sendet genehmigte Anträge digital zurück** — Claudia Rehm schickt den genehmigten DR-Antrag als gescanntes PDF per Email an den/die Antragsteller*in
3. **Originalbelege beim Reisenden** — 5 Jahre Aufbewahrungspflicht liegt jetzt bei den Dienstreisenden selbst

### Digitaler Einreichungsprozess

#### Phase 1: Antrag einreichen
1. DR-001 ausfüllen (PDF)
2. DR-003 Kostenkalkulation ausfüllen (Excel)
3. Beides ausdrucken, unterschreiben
4. **An Franziska Schuchort (InfAU Office) geben** — per Hand oder per Email an caad@architektur.uni-weimar.de. Franzi leitet den Antrag an das Dekanat (Claudia Rehm) weiter. Bei Email-Einreichung Franzi mindestens in CC setzen.
5. Dekanat leitet an Dekan zur Genehmigung weiter
6. **Genehmigter Antrag kommt per Email zurück** (von Claudia Rehm als Scan)

#### Phase 2: Abrechnung einreichen
1. DR-004 ausfüllen (PDF)
2. Unterschreiben (digital oder ausdrucken+scannen)
3. Alle Unterlagen als PDF scannen:
   - Ausgefüllte Reisekostenrechnung (DR-004)
   - Genehmigter Dienstreiseantrag (DR-001) — den man vom Dekanat per Email erhalten hat
   - Alle Belege (Fahrkarten, Hotelrechnung, Tickets etc.)
4. **Per Email senden** an: `Reisekostenabrechnung@uni-weimar.de`

### Email-Format für Abrechnung

**An**: Reisekostenabrechnung@uni-weimar.de
**Betreff**: `Name, Vorname; Reisezeitraum; Geschäftsort; Abrechnungsobjekt`

Beispiel: `Name, Vorname; 24.-25.03.2026; Geschäftsort; Abrechnungsobjekt`

**Anhänge**:
- DR-004 (ausgefüllt, unterschrieben)
- DR-001 (genehmigt, gescannt)
- Alle Belege als PDF

**Text-Vorlage**:
```
Sehr geehrte Damen und Herren,

anbei übersende ich Ihnen meine Reisekostenabrechnung für die Dienstreise nach [ORT] vom [DATUM_VON] bis [DATUM_BIS].

Beigefügt sind:
- Reisekostenrechnung (DR-004)
- Genehmigter Dienstreiseantrag (DR-001)
- [ANZAHL] Belege (Fahrkarten, Hotelrechnung, [weitere])

Mit freundlichen Grüßen
[NAME]
[FAKULTÄT/BEREICH]
```

### Email-Format für Abschlag-Antrag

**An**: Reisekostenabrechnung@uni-weimar.de
**Betreff**: `Abschlag — Name, Vorname; Reisezeitraum; Geschäftsort; Abrechnungsobjekt`

---

## Ablauf-Checkliste für den Skill

### Vor der Reise (Antrag)
1. ☐ DR-001 ausfüllen
2. ☐ DR-003 Kostenkalkulation erstellen
3. ☐ Beide Dokumente ausdrucken und unterschreiben
4. ☐ An Franziska Schuchort (InfAU Office, caad@architektur.uni-weimar.de) geben — sie leitet an Dekanat weiter
5. ☐ Auf genehmigten Antrag per Email warten
6. ☐ Ggf. Fahrkarten über BUW-Bucher bestellen
7. ☐ Ggf. Abschlag beantragen (ab 100 EUR, max 80%)

### Nach der Reise (Abrechnung)
1. ☐ Genehmigten Antrag (Email vom Dekanat) bereithalten
2. ☐ Alle Belege sammeln und als PDF scannen
3. ☐ DR-004 ausfüllen
4. ☐ DR-004 unterschreiben
5. ☐ Email an Reisekostenabrechnung@uni-weimar.de mit allen Anhängen senden
6. ☐ **Originalbelege 5 Jahre aufbewahren!** (eigene Verantwortung)
7. ☐ Frist beachten: **max. 3 Monate nach Reiseende**

### Nächste Schritte — Was der Skill dem User sagen soll

**Nach Antrag-Ausfüllung**:
> "Drucke den Antrag (DR-001) und die Kostenkalkulation (DR-003) aus, unterschreibe beides und gib die Unterlagen bei **Franziska Schuchort (InfAU Office)** ab — oder sende sie per Email an caad@architektur.uni-weimar.de. Franzi leitet alles an das Dekanat weiter. Du bekommst den genehmigten Antrag dann per Email zurück — hebe diese Email gut auf, du brauchst sie für die Abrechnung!"

**Nach Abrechnung-Ausfüllung**:
> "Unterschreibe die Reisekostenrechnung (DR-004), scanne sie als PDF, und sende alles per Email an Reisekostenabrechnung@uni-weimar.de. Ich habe dir unten einen Email-Entwurf vorbereitet. Vergiss nicht: **Originalbelege 5 Jahre aufbewahren!**"
